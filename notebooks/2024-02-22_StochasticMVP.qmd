...
[Previously](./2024-02-02_CostEstimateMVP.html), we have worked out a deterministic model of the sequencing depth required to detect a novel pandemic virus by the time it reaches a fixed cumulative incidence (number of people ever infected).
...
In contrast, our deterministic model predicts that we dectect when $\mu = 2$.
...
Putting these results together suggests that regardless of the regime, we expect the mean at detection to be roughly 1.5 to 3.0 times higher under a stochastic model than under the deterministic approximation.
