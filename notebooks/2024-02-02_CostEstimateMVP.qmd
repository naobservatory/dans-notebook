...
From here on, it will simplify thing to specify two concrete sampling schemes.
...
2. Grab sampling: $\rho_i(t) = \delta(t - t_i)$, i.e., the whole sample is collected at $t_i$.
...
One complication is that the sample $i$ that $K_i \geq \hat{K}$ depends on the precise timing of our samples.
...
2. The residual time waiting for the next sample to be collected. Because we have no reason to expect this waiting time to take any particular value, we average it over a uniform distribution.
...
\end{align}
$$
The the third term in parentheses is the cost incurred from having widely spaced samples.
...
Substituting and rearranging gives the sequencing effort required to detect by cumulative incidence $\hat{c}$:
$$
