...
experiments <- c(
  "[2023-10-12] Settled Solids Protocol Development, Vortex Time and Centrifuge Settings",
...
amp_data <- list.files(
  ...
  list_rbind() |>
  mutate(Target = if_else(Target == "PMMV", "PMMoV", Target)) |>
  left_join(tidy_data, by = join_by(plate, Well, `Well Position`, Sample, Omit, Target)) |>
  glimpse()
```
