...
```{r}
plot_amp_with_ruler <- function(target, y0_from) {
  y0 <- 10^seq(from = y0_from, by = -1, length.out = 5)
  ruler <- crossing(`Cycle Number` = amp_data$`Cycle Number`, y0 = y0) |>
...
```{r}
plot_amp_with_ruler("16S", -6)
plot_amp_with_ruler("CrA", -5)
plot_amp_with_ruler("Noro", -8)
plot_amp_with_ruler("Cov2", -9)
```
